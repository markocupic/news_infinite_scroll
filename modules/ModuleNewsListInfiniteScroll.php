...
class ModuleNewsListInfiniteScroll extends \ModuleNews
{
	 ...
	 * @var string
	 */
	protected $strTemplate = 'mod_newslist_infinite_scroll';
	...
	public function generate()
	{
		...
		if(!\Input::get('infiniteScrollNewsLoader'))
		{
			//return '';
		}
	...
	protected function compile()
	{
		...
		if(!\Input::get('infiniteScrollNewsLoader'))
		{
			...
			while($objArticles->next()){
				$html .= '<div style="display:none;" class="inifinite-scroll-news-to-load-' . $this->id . '" data-news-id="' . $objArticles->id . '"></div>' . "\r\n";
			}
			$this->Template->itemsToLoad = $html;
		}else{
			if(\Input::get('ids')){
				$ids = explode('-', \Input::get('ids'));
				$opt = array();
				$objArticles = \NewsModel::findMultipleByIds($ids, $opt);
			}else{
				$ids = array();
				$objArticles = $this->fetchItems($this->news_archives, $blnFeatured, ($limit ?: 0), $offset);
			}
